...
def main():
    ...
    model = UNetResNet(34, 1, num_filters=32, dropout_2d=0.2, pretrained=True, is_deconv=False).to(device)
    # model.load_state_dict(torch.load("/storage/model.pth", map_location=device))

    swa_model = UNetResNet(34, 1, num_filters=32, dropout_2d=0.2, pretrained=True, is_deconv=False).to(device)
